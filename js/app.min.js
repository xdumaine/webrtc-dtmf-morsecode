...
var messageOutput = document.querySelector('div#morsecode');

messageInput.onkeyup = function () {
    var value = messageInput.value;
    var morseCode = morseify(value);
...
function iceCallback1(event) {
    if (event.candidate) {
        pc2.addIceCandidate(new RTCIceCandidate(event.candidate), onAddIceCandidateSuccess, onAddIceCandidateError);
...
function iceCallback2(event) {
    if (event.candidate) {
        pc1.addIceCandidate(new RTCIceCandidate(event.candidate), onAddIceCandidateSuccess, onAddIceCandidateError);
...
function sendMessage(message) {
    if (dtmfSender) {
        (function () {
            var morse = morseify(message);
            var tones = [];
            morse.forEach(function (code) {
                console.log('sending code', code);
                for (var i = 0; i < code.length; i++) {
                    var char = code.charAt(i);
                    var duration = char === '.' ? 100 : 300;
                    tones.push({
                        tone: 8,
                        duration: duration,
                        delay: 50,
